...
def get_report_pdf(doc, consolidated=True):
	...
	template_path = (
		"tiejan/tiejan_customizations/doctype/soa_tiejan/soa_tiejan.html"
	)
	...
	for entry in doc.customers:
		...
		for item in res:
			if "sales_person_x" in item:
				res_with_sales_person.append(item)
		...
		def dict_clean(items):
			...
			for key, value in items:
				...
				result[key] = value
			return result
		dict_str = json.dumps(res_with_sales_person, indent=4, sort_keys=True, default=str)
...
def download_statements(document_name):
	doc = frappe.get_doc("SOA Tiejan", document_name)
