...
def get_gl_entries(filters, accounting_dimensions):
	...
	for entry in gl_entries:
		...
		if(entry['against_voucher_type'] == 'Sales Invoice'):
			sales_person_x = frappe.db.sql("""SELECT GROUP_CONCAT(sales_person) FROM `tabSales Team` WHERE parent = '%(sinv)s'"""%{"sinv" :entry['voucher_no'] }, as_list = True)
			if sales_person_x:
				sales_person_x = sales_person_x[0][0]
			entry.update({"sales_person_x" : sales_person_x})
